...
public class HorizontalSlashParticle extends SpriteBillboardParticle {
    public final SpriteProvider spriteProvider;

    public HorizontalSlashParticle(ClientWorld world, double x, double y, double z, double velocityX, double velocityY, double velocityZ, SpriteProvider spriteProvider) {
        super(world, x, y, z, velocityX, velocityY, velocityZ);
        this.velocityX = 0;
        this.velocityY = 0;
        this.velocityZ = 0;
        this.spriteProvider = spriteProvider;
        float modifier = (float) new Random().nextInt(60) / 100;
        this.scale = 0.7f + modifier;
        this.maxAge = 5;
    }

    float modifierY = new Random().nextInt(35);
    float modifierZ = new Random().nextInt(30);
    ...
    public void buildGeometry(VertexConsumer vertexConsumer, Camera camera, float tickDelta) {
        ...
        for (int k = 0; k < 4; ++k) {
            ...
            Vec3f2.rotate(new Quaternionf().rotateXYZ((float) Math.toRadians(0f), -110f+modifierY, 0f+modifierZ));
            Vec3f2.mul(j);
            Vec3f2.add(f, g, h);
        }
    ...
    public static class DefaultFactory implements ParticleFactory<SimpleParticleType> {
        ...
        public Particle createParticle(SimpleParticleType parameters, ClientWorld world, double x, double y, double z, double velocityX, double velocityY, double velocityZ) {
            return new HorizontalSlashParticle(world, x, y, z, velocityX, velocityY, velocityZ, this.spriteProvider);
        }
    }
}
